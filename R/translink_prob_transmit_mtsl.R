...
translink_prob_transmit_mtsl <- function(specificity, # specificity of the linkage criteria
  ...
  (1 - exp(-rho * sensitivity * (R + 1))) /
    (1 - ((specificity^(M - 1)) * exp(rho * (R + 1) * ((1 - sensitivity) / specificity - 1))))
}
