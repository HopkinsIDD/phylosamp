---
title: <font size="5"> Estimating bias in observed variant prevalence </font>
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{V1: Estimating bias in observed variant prevalence}
...
The combination of the factors that may affect the detection and characterization of a particular variant can be summarized in a parameter termed the _coefficient of detection_, which in turn can be estimated from the following parameters:
...
|   **$\alpha_{V_1}$**   |       alpha_v1       | the probability that an infection caused by variant 1 is asymptomatic           |
|   **$\alpha_{V_2}$**   |       alpha_v2       | the probability that an infection caused by variant 2 is asymptomatic           |
|   **$\beta_{a}$**   |       beta_a       | the probability of testing an asymptomatic infection (any variant)           |
|   **$\beta_{s}$**   |       beta_s       | the probability of testing a symptomatic infection (any variant)           |

While it may be difficult to estimate all of the parameters needed to estimate this coefficient, we find that only the *ratio* in the coefficients of detection for two variants (more commonly, the variant of interest and the values for the rest of the pathogen population) is necessary for understanding the biases in a sample. Therefore, it may not be necessary to estimate the true value of each parameter, provided we can estimate the ratio between their values for the two variants in question. We can use the `calc_cod_ratio()` function to estimate the coefficient of detection ratio, only providing parameters which are expected to differ between variants. The ratio between any variants not provided is assumed to be equal to one.
...
calc_expected_mbias(p_v1=0.2, c_ratio=1.367)

```

As expected, this value is the result from the `calc_observed_freq()` function divided by the true underlying prevalence. This value tells use that we are overestimating the variant of interest frequency by roughly 25%.