...
vignette: >
  %\VignetteIndexEntry{V3: Estimating the probability of detecting a variant (cross-sectional)}
...
|   **$n$**   |       n       | the sample size           |
|   **$\omega$**   |       omega       | the probability of sequencing success           |
...
calc_prob_detect(p_v1=0.02, n=100, omega=0.8, c_ratio=c1_c2)

```

In other words, we have an 89% probability of detecting a variant at 2% (or higher) in a population given a sample size 100 sequences. In this calculation, we assumed that only 80% ($\omega=0.8$) of samples sequenced (or otherwise characterized) are successful, leading to an effective sample size of 80 samples. We also calculated a coefficient of detection ratio ($\frac{C_{V_1}}{C_{V_2}}$) of 1.368, which increased our confidence in detecting the variant, since we expect it to be enriched in the population of detected infections.

In some cases, we may be more interested in correctly estimating the variant frequency than simply stating its presence or absence in the population. In this case, we can use the `calc_prob_prev()` function to determine our confidence in our estimate of variant prevalence given a fixed sample size. This function requires the user to specific a slightly different set of parameters:
